...
이 <span style="color:#B8860B">**View 클래스**</span>를 상속 받는다. 

아래와 같이 스스로 메소드를 정의할 수 있다.
...
RedirectView는 주어진 URL로 리다이렉트 시켜주는 제네릭 뷰다. 그래서 URL속성이 필수이다.

URL이 없다면 URL 패턴명을 사용하면 된다.
...
DetailView를 사용할 경우 테이블은 뷰 클래스에서 지정하고, 레코드 검색용 키는
...
그래서 CreateView는 <span style="color:#4682B4">**FormView의 기능을 포함**</span>하고 있다.

CreateView는 항상 작업 대상이 되는 테이블이 있으므로, 그 테이블 정의로부터 폼을 만들 수 있다.
