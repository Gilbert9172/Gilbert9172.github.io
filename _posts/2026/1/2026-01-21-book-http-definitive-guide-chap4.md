...
- 이 구조에서는 ACK가 편승할 수 있는 반대 방향 데이터가 적어, 확인응답 지연 알고리즘으로 인한 **추가 지연이 자주 발생**한다.
...
- 이 알고리즘은 RFC 896  
  *“Congestion Control in IP/TCP Internetworks”* 에 정의되어 있다.
...
- 네이글 알고리즘은 **확인응답 지연(delayed ACK)** 과 함께 사용될 경우,
  서로를 기다리며 성능이 급격히 저하되는 현상이 발생한다.
