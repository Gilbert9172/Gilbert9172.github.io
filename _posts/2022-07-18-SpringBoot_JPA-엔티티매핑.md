...
☸︎ `@Entity`가 붙은 클래스는 JPA가 관리, 엔티티라고 한다.

☸︎ JPA를 사용해서 테이블과 매핑할 클래스는 `@Entity` 필수

☸︎ 기본 생성자 필수(파라미터가 없는 pulic 또는 protected 생성자)

☸︎ final 클래스, enum, interface, inner 클래스 사용 ❌
...
☸︎ 애플리케이션 실행 시점에 자동으로 DDL을 생성

☸︎ 테이블 중심에서 객체 중심으로 개발 가능

☸︎ ***<span style="background-color:yellow">데이터베이스 방언을 활용해서 데이터베이스에 맞는 적절한 DDL 생성</span>***
...
***@Enumerated*** ***<span style="color:red">(ORDINAL 사용금지)</span>***
...
그리고 Sequence 전략은 IDENTITY 전략과는 다르게 버퍼기능을 지원한다.
...
***<span style="background-color:yellow">Long Type + 대체키 + 키 생성 전략 사용</span>***
