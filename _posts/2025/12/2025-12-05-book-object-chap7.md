...
#### {% include i.html %} 4-1. 추상 데이터 타입

> 추상 데이터 타입

- 프로시저 추상화 대신 **데이터 추상화**를 기반으로 소프트웨러를 개발하게 한 최초의 발검음
- 시스템의 상태를 저장하라 데이터를 표현한다.
...
#### {% include i.html %} 5-1. 클래스는 추상 데이터 타입인가? 

> 클래스와 데이터 타입의 가장 핵심적인 차이

- 클래스는 상속과 다형성을 지원하는 데 비해 추상 데이터 타입은 지원하지 못한다.
- 추상 데이터 타입 기반의 프로그래밍 패러다임을 **객체기반(Object-Based) 프로그래밍**이라고 부르기도 함. 

<br>

> 핵심 내용

- 추상 데이터 타입은 오퍼레이션을 기준으로 타입(type)을 추상화한 것
  - 하나의 대표적인 타입(Employee)이 세부적인 타입(정규직, 아르바이트)을 감춘다.
  - 따라서 타입 추상화는 **오퍼레이선을 기준으로 타입을 통합**하는 데이터 추상화 기법
- 클래스(객체지향)는 타입을 기준으로 절차(procedure)를 추상화한 것
  - **타입을 기준으로 오퍼레이션을 통합**한다.
  - 객체지향은 정규 직원과 아르바이트 직원 각각에 대한 클래스를 정의하고, 각 클래스에서 오퍼레이션을 적절할게 구현한다.
  - 즉, 동일한 메시지에 다르게 반응한다. 이것이 바로 다형성

<br>

#### {% include i.html %} 5-3. 변경을 기준으로 선택하라.

> 클래스가 추상 데이터 타입의 개념을 따르는지 확인하는 법

- 클래스 내부에 인스턴스의 타입을 표현하는 변수가 있는지를 살펴봐라.
    - 객체지향에서는 타입 변수를 이용한 조건문을 다형성으로 대체한다.
    - 조건문을 기피하는 이유는 **변경** 때문이다.
...
> 항상 추상 데이터 타입이 최악인가? NO
 
- 타입 추가라는 변경의 압력이 더 강한 경우에는 객체지향을 
- 주된 압력이 오퍼레이션을 추가하는 것이라면 추상 데이터 타입을

<br>

#### {% include i.html %} 5-4. 협력이 중요하다.
